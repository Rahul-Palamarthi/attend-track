...
export default function Subject() {
	...
	const data = {
		datasets: [
			{
				data: [subject.attendedClasses, subject.totalClasses === 0 ? 2 : subject.totalClasses - subject.attendedClasses],
	...
	function handleSubjectDelect() {
		const res = window.confirm('Are you sure you want to delete this subject');
		if (!res) {
			return;
		}
	...
	function handleAttendedClassesDecrement() {
		setSubject((currState) => ({
			...currState,
			attendedClasses: currState.attendedClasses !== 0 ? currState.attendedClasses - 1 : 0,
		}));
	}
	...
	function handleTotalClassesDecrement() {
		...
		setSubject((currState) => ({
			...currState,
			totalClasses: currState.totalClasses !== 0 ? currState.totalClasses - 1 : 0,
		}));
	}
	...
	useEffect(() => {
		function findPercentage() {
			return Number(((subject.attendedClasses / subject.totalClasses) * 100).toFixed(0));
		}
	...
	useEffect(() => {
		if (subject.totalClasses !== 0) {
			setPercentage(Number(((subject.attendedClasses / subject.totalClasses) * 100).toFixed(0)));
		}
					...
					{subject === undefined ? (
						...
						<>
							...
							<div className='each-subject-chart-wrapper'>
								<div className='each-subject-chart' data-percentage={`${percentage}%`}>
								...
								<div className='chart-info'>
									<p
										...
										data-info={'Present'}
										style={{
											backgroundColor: `${colors[(sub.id - 1) % colors.length]}`,
										}}
									/>
									<p
							...
							<div className='classes-wrapper'>
								<p>Attendenced classes: {subject.attendedClasses}</p>
								<div>
									<button type='button' className='border' onClick={handleAttendedClassesDecrement}>
										-
									</button>
									<button type='button' className='border' onClick={handleAttendedClassesIncrement}>
										+
									</button>
								</div>
								<p>total classes: {subject.totalClasses}</p>
								<div>
									<button type='button' className='border' onClick={handleTotalClassesDecrement}>
										-
									</button>

									<button type='button' className='border' onClick={handleTotalClassesIncrement}>
										+
									</button>
								</div>
							</div>
							<hr
								style={{
									border: `1.25px solid ${colors[(sub.id - 1) % colors.length]}`,
								}}
							/>
							<div className='status-wrapper'>
								<button type='button' className='border' onClick={handlePresent}>
									Present
								</button>
								<button type='button' className='border' onClick={handleAbsent}>
									Absent
								</button>
							</div>
							<div className='outro'>
								...
								<button type='button' className='border' onClick={handleSubjectDelect}>
									Delete Subject
								</button>
							</div>
						</>
