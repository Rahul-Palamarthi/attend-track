...
export default function NewSubject() {
	...
	function handleCreateSubject() {
		...
		for (let i = 0; i < prevSubjects.length; i++) {
			if (prevSubjects[i].subject.toLowerCase() === subjectValue.toLowerCase()) {
	...
	return (
		<>
			<section className='create-subject-wrapper'>
				<div>
					<p>
						<input ref={subject} type='text' name='subject' id='subject' placeholder='Subject' autoComplete='off' />
					</p>
					<p>
						<input
							ref={attendedClasses}
					...
					<p>
						<input ref={totalClasses} type='number' name='totalClasses' id='totalClasses' placeholder='Total Classes' autoComplete='off' />
					</p>

					<hr />
					<button className='border' type='button' onClick={handleCreateSubject}>
						Create Subject
					</button>
					<Link to={'/'}>
